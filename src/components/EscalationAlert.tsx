...
type Active = { code?: string; subject?: string } | null;

/**
 * Global overlay: when a ticket ESCALATES on the active hotel (server emits
 * "ticket.escalated"), a looping vibration starts and this banner shows until
 * the user taps Stop. Mounted above navigation so it appears on any screen.
 */
export function EscalationAlert() {
  ...
  useRealtime(status === "signedIn" ? hotelId : null, (e) => {
    if (e.type === "ticket.escalated") {
      const p = (e.payload ?? {}) as { code?: string; subject?: string };
      setActive({ code: p.code, subject: p.subject });
      startEscalationLoop();
    }
  });
  ...
  return (
    <SafeAreaView style={styles.wrap} pointerEvents="box-none">
      <View style={styles.banner}>
        ...
        <View style={{ flex: 1 }}>
          <Text style={styles.title}>Escalated {active.code ? `· ${active.code}` : ""}</Text>
